  ...
  const { node, close } = sheet('Player display', body, sheetNode => {
    on(sheetNode, 'click', '[data-player-save]', () => {
      ...
      for (const c of list) {
        ...
        const revealConditions = Boolean(qs(`[data-player-conditions="${CSS.escape(c.id)}"]`, sheetNode)?.checked);
        next[c.id] = {
