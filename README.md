...
band. The character picker uses `personalLootSelection()` for two options from categories
that do not depend on a particular weapon, armour proficiency, casting tradition, or class
feature. Giving one awards it directly to that character and deducts its exact value from
the remaining level budget; it never suggests an item that would overspend that budget. If
the loot roster is empty it is filled from the committed campaign characters first.
